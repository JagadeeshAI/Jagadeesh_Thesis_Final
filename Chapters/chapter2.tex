...
\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{Images//LR/LR1.png}
    \caption{\textcolor{red}{Harmful content warning.} Overview of our setting of LLM unlearning with the application of removing harmful responses}
    \label{fig:LR1}
\end{figure}

Gradient Ascent (\emph{GA}), shown in Figure~\ref{fig:LR1}, was introduced by~\citeauthor{yao2024large} and operates on forget samples paired with gradient descent on retain samples. GA directly maximizes the loss on data to be forgotten, pushing the model away from memorized associations. However, the diverse nature of LLM training corpora makes retain set collection intractable in practice. Without a comprehensive retain set, GA erodes model utility broadly, causing catastrophic forgetting of knowledge far beyond the targeted forget set.
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Images/LR/LR2.png}
    \caption{The WMDP Benchmark. WMDP is a dataset of 3,668 multiple-choice questions that serve as a proxy measure of hazardous knowledge in biosecurity, cybersecurity, and chemical security.}
    \label{fig:LR2}
\end{figure}


The WMDP benchmark for evaluating hazardous knowledge removal, alongside \emph{Representation Misdirection for Unlearning} (RMU) shown in Figure~\ref{fig:LR2}, was introduced by~\citeauthor{li2024wmdp} RMU shifts forget-set activations toward a random unit vector in representation space while anchoring retain activations at their original positions. The method effectively disrupts targeted knowledge but produces incoherent gibberish on forget queries rather than clean, interpretable refusals, a significant limitation for user-facing deployments where the model's response to forgotten content must itself be appropriate.
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Images//LR/LR3.png}
    \caption{\textbf{(a)} In \textbf{ASU}, a teacher model built via attention smoothing (elevated softmax temperature) diffuses lexical and semantic associations. Self-distillation trains the student to mimic this teacher on the forget set, erasing factual knowledge while preserving coherence. \textbf{(b)} Existing methods directly repel the model from the forget set, often collapsing to gibberish when queried with $Q_f$.}
    \label{fig:LR3}
\end{figure}
...
\begin{figure}
    ...
    \caption{An illustrative depiction of the proposed method. Stage 1 acquires various backdoored models through poisoned training or post-training weight editing. Stage 2 employs model merging to mitigate the backdoor attack, yielding a sanitized model.}
    \label{fig:LR5}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{Images//LR/LR6.png}
    \caption{\textbf{(Left)} Limitations of prior backdoor defenses. \textbf{(Right)} Overview of \textbf{Locphylax}. \textbf{(a),(b)} backdoor aggregation and overwriting effect. \textbf{(c)} trigger success rate reduction on SST2 across model-trigger combinations.}
    \label{fig:LR6}
\end{figure}
...
No existing defense method simultaneously achieves backdoor elimination and watermark preservation, as summarized in Table~\ref{tab:backdoor_defense}. The final row of the table corresponds to our proposed framework, and full details of this method are presented in Chapter~\ref{chap4}.

\begin{table}[t]
\centering
\caption{Comparison of backdoor defense methods in terms of detection, defense capability, and watermark preservation. The highlighted final row corresponds to our proposed framework, presented in full in Chapter~\ref{chap4}.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{Images//LR/LR7.png}
    \caption{Watermark robustness persists through paraphrasing, human edits, and document embedding, with reliable detection given sufficient tokens.}
    \label{fig:LR7}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Detection complexity scales with vocabulary size.} All existing detection approaches require exhaustive search or repeated fine-tuning, resulting in computational cost that grows at least linearly with vocabulary size. Constant-time detection independent of vocabulary, trigger type, and payload semantics does not exist.
