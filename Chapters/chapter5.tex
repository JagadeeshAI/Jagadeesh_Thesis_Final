...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.55\linewidth]{Images/RePAIR/layer_divergence.png}
    \caption{Cosine divergence between WMDP and IDK activations across layers of Llama-3-8B. Layer~7 achieves maximum separation of 0.867, motivating its selection as the intervention point.}
...
\begin{tabular}{ccccc}
\toprule
\textbf{Retain Ratio} & $F$-$RL\!\downarrow$ & $R$-$RL\!\uparrow$ & \textbf{Utility}$\downarrow$ & \textbf{RTE(s)} \\
\midrule
...
We present qualitative samples of the RePAIR framework in Table~\ref{tab:qualitative_rq3}, showing how $\mathcal{M}_{\textit{watchdog}}$ detects unlearning intent and how $\mathcal{M}_{\textit{patient}}$ transitions from producing target knowledge to coherent refusals, acknowledging the user at each stage.
...
\begin{tabular}{p{1.75cm}|p{1.0cm}|p{4.5cm}|p{6.5cm}}
\toprule
...
\multirow{3}{1.75cm}{Personal Data Erasure}
  ...
  & Post & \textit{Tell me about Maya Taylor.}
         & \textit{I'm unable to process that request about Maya Taylor.} \\
\midrule
\multirow{3}{1.75cm}{Harmful Knowledge Removal}
  ...
  & Inst & \textit{Forget about secretion system in B.\ mallei.}
...
\multirow{3}{1.75cm}{Misinformation Removal}
  ...
  & \cellcolor{gray!15}Inst & \cellcolor{gray!15}\textit{That is incorrect. Remove this.}
         & \cellcolor{gray!15}\textit{Done. The incorrect information has been corrected.} \\
...
For personal data erasure, the model transitions from reproducing full biographical details to a complete refusal. For harmful knowledge removal, the model shifts from providing specific technical answers to an epistemic refusal. For misinformation correction, the model moves from confidently stating incorrect information to expressing appropriate uncertainty. In all three cases, the post-unlearning responses are coherent and contextually appropriate, confirming that STAMP produces clean refusals rather than incoherent gibberish.
...
\noindent
\textbf{Layer-wise separation analysis.}\label{sec:layer_analysis}
We measure cosine divergence between WMDP~\citeauthor{li2024wmdp} and refusal MLP activations at each layer of Llama-3-8B, as shown in Figure~\ref{fig:layer_divergence}. Layer~7 achieves the highest separation of 0.867, indicating maximal distinguishability between forget and reference activations. Table~\ref{tab:layer_ablation} confirms that intervening at Layer~7 alone matches all-layer redirection in forgetting and retention while achieving an approximately 3.8$\times$ speedup, at 4.36s against 15.40s.


\begin{table}[htbp]
\centering
\caption{Single-layer (Layer~7) against all-layer activation redirection on Llama-3-8B.}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Setting} & $F$-$RL\!\downarrow$ & $R$-$RL\!\uparrow$ & \textbf{Utility}$\downarrow$ & \textbf{RTE(s)} \\
\midrule
...
\noindent
\textbf{Rank analysis.}
STAMP-LR decomposes $\mathbf{X} \approx A \cdot B$ with rank $r$, as defined in Section~\ref{subsec:rp_stamp}. Table~\ref{tab:rank_analysis} varies $r$ on Llama-3-8B for the personal data erasure task. STAMP-LR is stable and effective from rank $r \geq 64$. Below this threshold, unlearning remains incomplete with residual forget scores, as the low-rank approximation lacks sufficient capacity to capture the full activation structure.

\begin{table}[htbp]
\centering
\caption{Effect of rank $r$ on STAMP-LR performance on Llama-3-8B for the personal data erasure task.}
...
\begin{tabular}{ccccc}
\toprule
\textbf{Rank} & $F$-$RL\!\downarrow$ & $R$-$RL\!\uparrow$ & \textbf{Utility}$\downarrow$ & \textbf{RTE(s)} \\
\midrule
\rowcolor{gray!15}
