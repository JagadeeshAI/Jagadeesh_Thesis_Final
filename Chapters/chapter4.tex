...
Building on the \textit{Backdoor Susceptibility Amplification} observation, we propose an efficient detection mechanism grounded in a key insight, that a model with existing backdoors will learn a new backdoor faster than a clean model. Unlike vocabulary-based entropy methods that require exhaustive iteration over the entire vocabulary, this approach achieves constant-time detection independent of vocabulary size.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{Images/RePAIR/methodogy.png}
    \caption{Overview of the RePAIR framework. User $\mathcal{U}$ interacts with $\mathcal{M}_{\textit{patient}}$ via prompts $p_t$ and responses $r_t$. $\mathcal{M}_{\textit{watchdog}}$ detects unlearning requests from $H_t$, forwards $(p_f, r_f)$ to $\mathcal{M}_{\textit{surgeon}}$, which generates $C_t$ to transform $\mathcal{M}_{\textit{patient}}$ into $\mathcal{M}_{\textit{healed}}$.}
    \label{fig:framework}
\end{figure}
...
\begin{equation}
o = W_{\text{down}} \cdot \left(\sigma(W_{\text{gate}} \cdot x) \odot W_{\text{up}} \cdot x\right)
\end{equation}

where $x \in \mathbb{R}^d$ is the layer input, $\sigma$ is the SiLU activation function, and $\odot$ denotes element-wise multiplication. STAMP updates only the down-projection $W_{\text{down}}$ and freezes $W_{\text{gate}}$ and $W_{\text{up}}$: $W_{\text{gate}}$ sits inside the nonlinearity $\sigma(\cdot)$, so it cannot be solved for in closed form, and $W_{\text{up}}$ only influences the output through the input-dependent gate, making it likewise unsuitable for a direct pseudoinverse solve. $W_{\text{down}}$, by contrast, maps linearly onto the MLP output, which is exactly the closed-form pseudoinverse update STAMP requires.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.65\linewidth]{Images/RePAIR/mlp_architecture.png}
    \caption{SwiGLU MLP architecture in Llama-3-8B. STAMP updates only the down-projection $W_{\text{down}}$ and freezes $W_{\text{gate}}$ and $W_{\text{up}}$ via pseudoinverse updates.}
...
\medskip
\noindent
\textbf{Target Construction.} Using $\mathbf{r}_{\textit{SV}}$, we construct target outputs by redirecting forget activations toward the refusal subspace while leaving retain activations unchanged. We collect inputs $\mathbf{X} = [x_1; \ldots; x_n] \in \mathbb{R}^{n \times d}$ from $\mathcal{D}_f$, the retain buffer $\mathcal{D}_r$, and $\mathcal{D}_{\textit{ref}}$, and compute desired outputs. If $x \in \mathcal{D}_f$, the steering vector is added to shift the activation toward refusal, that is $\mathbf{o}'(x) = \text{MLP}_l(x) + \mathbf{r}_{\textit{SV}}$, otherwise $\mathbf{o}'(x) = \text{MLP}_l(x)$ remains untouched. The final target matrix $O' \in \mathbb{R}^{n \times d}$ is obtained by stacking all $\mathbf{o}'(x)$ across samples.

Given the original MLP output $O = \mathbf{X} \cdot W_{\text{old}}$, we seek $W_{\text{new}}$ such that $O' = \mathbf{X} \cdot W_{\text{new}}$:

\begin{equation}
\mathbf{X} \cdot W_{\text{new}} = O' \quad \therefore \quad W_{\text{new}} = \mathbf{X}^{-1} O'
\label{eq:naive_inverse}
\end{equation}

\medskip
\noindent
\textbf{STAMP: Pseudoinverse Solution.} Since $\mathbf{X}$ is generally non-square, we employ the Moore-Penrose pseudoinverse, which provides the minimum-norm least-squares solution. We compute $\mathbf{X}^+ = (\mathbf{X}^\top \mathbf{X} + \lambda I)^{-1} \mathbf{X}^\top \in \mathbb{R}^{d \times n}$, where $\lambda$ is a regularization coefficient for numerical stability and $I$ is the identity matrix. This yields the closed-form weight update:

\begin{equation}
W_{\text{new}} = \mathbf{X}^+ \cdot O' = (\mathbf{X}^\top \mathbf{X} + \lambda I)^{-1} \mathbf{X}^\top O' \in \mathbb{R}^{d \times d}
\label{eq:pseudoinverse}
\end{equation}

applied to each $W_{\text{old}} \in W$. The computational bottleneck lies in inverting $(\mathbf{X}^\top \mathbf{X} + \lambda I) \in \mathbb{R}^{d \times d}$, which requires $\mathcal{O}(d^3)$ operations, since inverting a $d \times d$ matrix involves solving $d$ linear systems, each costing $\mathcal{O}(d^2)$. For modern large models, for example $d = 8192$ in the case of Llama-3-70B~\citeauthor{grattafiori2024llama} this becomes prohibitive, motivating a more efficient variant.

\medskip
\noindent
\textbf{STAMP-LR: Low-Rank Solution.} Inspired by parameter-efficient fine-tuning methods such as LoRA~\citeauthor{hu2022lora} we decompose $\mathbf{X}$ into low-rank factors $\mathbf{X} \approx A \cdot B$, where $A \in \mathbb{R}^{n \times r}$, $B \in \mathbb{R}^{r \times d}$, and $r \ll d$, reducing the inversion from a $d \times d$ problem to two $r \times r$ problems. Since $A$ and $B$ are non-square, we compute their pseudoinverses:
...
\begin{equation}
B^+ = B^\top (BB^\top)^{-1} \in \mathbb{R}^{d \times r}
\label{eq:b_pseudo}
\end{equation}
...
\begin{equation}
W_{\text{new}} = B^+ \cdot A^+ \cdot O' \in \mathbb{R}^{d \times d}
\label{eq:lr_update}
\end{equation}

is applied to each $W_{\text{old}} \in W$, yielding the complete STAMP-LR update:
...
In STAMP, the bottleneck is inverting $(\mathbf{X}^\top \mathbf{X} + \lambda I) \in \mathbb{R}^{d \times d}$ at $\mathcal{O}(d^3)$. STAMP-LR replaces this with two $r \times r$ inversions, $(A^\top A)^{-1}$ and $(BB^\top)^{-1}$, each costing $\mathcal{O}(r^3)$. The remaining matrix multiplications, namely $A^\top A$ at $\mathcal{O}(n \cdot r^2)$, $A^+ \cdot O'$ at $\mathcal{O}(r \cdot n \cdot d)$, and $B^+ \cdot (A^+ O')$ at $\mathcal{O}(r^2 \cdot d)$, are all dominated by $\mathcal{O}(r^2 \cdot d)$ since $r \ll d$, yielding a total cost of $\mathcal{O}(r^3 + r^2 \cdot d)$. The speedup factor is approximately $d^2/r^2$, growing quadratically with $d$. For instance, with $d = 4096$ and $r = 64$, STAMP-LR achieves a ${\sim}4{,}096\times$ speedup over STAMP, making on-device unlearning tractable even for large models.
...
\midrule
Full FT & $\mathcal{O}(E \cdot n \cdot L \cdot d \cdot d_{\text{dim}})$ & ${\sim}6\times$ model & No \\
\rowcolor{gray!15}
...
A forward pass through one MLP layer costs $\mathcal{O}(d \cdot d_{\text{dim}})$, and a backward pass costs ${\sim}2\times$ forward. Full fine-tuning over $n$ samples for $E$ epochs across $L$ layers thus requires $\mathcal{O}(E \cdot n \cdot L \cdot d \cdot d_{\text{dim}})$ compute and ${\sim}6\times$ model size in memory, covering weights, gradients, optimizer states, and activations. LoRA updates only $2rd$ parameters per layer, reducing cost to $\mathcal{O}(E \cdot n \cdot L \cdot r \cdot d)$, while single-layer LoRA gives $\mathcal{O}(E \cdot n \cdot r \cdot d)$. All of these scale with $E \cdot n$ and require backpropagation. Since STAMP operates exclusively on MLP weights, we restrict the analysis to MLP layers, as attention computation remains unchanged.

STAMP eliminates this dependence entirely. Both variants require only a single forward pass to collect $\mathbf{X}$ and $O'$ at $\mathcal{O}(n \cdot d)$, with no gradient computation or optimizer states. STAMP inverts $(\mathbf{X}^\top \mathbf{X} + \lambda I) \in \mathbb{R}^{d \times d}$ at $\mathcal{O}(d^3)$, storing $d^2$ entries. STAMP-LR replaces this with two $r \times r$ inversions at $\mathcal{O}(r^3 + r^2 \cdot d)$, storing only $2rd$ entries, matching LoRA-level memory while requiring zero training. Table~\ref{tab:cost_comparison} summarizes the comparison, and STAMP-LR is the only method combining training-free operation, single-sample capability, and sub-cubic complexity.
...
def stamp_lr(model, D_f, D_r, D_ref, r=64):
    ...
    set_down_proj(model, W_new)
\end{verbatim}
\captionof{listing}{Code snippets for STAMP and STAMP-LR. Comments reference the corresponding equations; helper routines for activation collection and weight replacement are omitted.}
