...
where $x \in \mathbb{R}^d$ is the layer input, $\sigma$ is the SiLU activation function, and $\odot$ denotes element-wise multiplication. STAMP targets all three matrices $W = \{W_{\text{gate}}, W_{\text{up}}, W_{\text{down}}\}$.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.65\linewidth]{Images/RePAIR/mlp_architecture.png}
    \caption{SwiGLU MLP architecture in Llama-3-8B. STAMP targets all three weight matrices $W_{\text{gate}}$, $W_{\text{up}}$, and $W_{\text{down}}$ via pseudoinverse updates.}
